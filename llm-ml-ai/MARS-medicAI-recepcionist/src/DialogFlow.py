...
## Dialogue Manager tiene que ser cambiado para que no printe nada pero nos guarde los objetos para poder rellenar slots.
...
it = 0; #Que condicion generar para acabar el bucle???? (Lo tenemos que pensar)
...
while dialog_act.current_DA != 'farewell': #!!!!!!!!!!!!!!!!!!!Que condicion generar para acabar el bucle???? (Lo tenemos que pensar)!!!!!!!!
    # text = None
    ...
    history.append(DH.History_Element('User',type_, sample))#Store qhat user says
    ...
    if dialog_act.current_DA == 'greeting':
       ...
       # engine.runAndWait()
        ...
        if intent.i_type == '':
            ...
            continue
        # if(more_info == True):
            # si es un intent valido again
            # palante con el nuevo intent
            # sino es un intent valido (eg solo una palabra "Pediatrics")
            # pillar ultimo intent del User, mirar los atributos, e identificar si la respuesta
            # del monolitico es valida para esos atributos, ponerlo a ese valor y ya tenemos
            # el intent again para tirar con el dialog manager
            # ponemos lack_info = False
        intent.pretty_print()
